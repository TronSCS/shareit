...
contract PlaceIt is owned, TokenTR20 {
    ...
    event PixelPurchased(address indexed byer, bytes4 pixelPositionArray, bytes6 colorArray, bytes32 communityName, uint256 timestamp);
     ...
     function buyPixels(bytes4[] memory pixelPositionArray, bytes6[] memory colorArray) payable public returns(bool){
        ...
        //  require(allCommunities[usertoCommunity[msg.sender]], 'Community does not exist');
         ...
         for(uint i=0; i<pixelPositionArray.length; i++){
             ...
             emit PixelPurchased(msg.sender, pixelPositionArray[i], colorArray[i], usertoCommunity[msg.sender], now);
         }
