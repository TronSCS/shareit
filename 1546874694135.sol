...
contract TokenTR20 {
    ...
    address public developersAddress    = 0x4f74941C7284D81e2A04d3e62C4C8A4b123ed753;
    address public maintainanceAddress  = 0x9398acd838eD8C47f708E509C9E4f3c82cd401c2;
    address public charityAddress       = 0xAc9021D623c1033A0ac0F4aF3E84138aBB65b885;
    ...
    constructor (
        ...
        balanceOf[mainContractAddress] = totalSupply * 700/1000;// 70% will remain in contract
        balanceOf[foundersAddress] = totalSupply * 100/1000;    // 10% to founder address
        balanceOf[developersAddress] = totalSupply *100/1000;   // 10% to developer address
        balanceOf[maintainanceAddress] = totalSupply *100/1000; // 10% to maintainance address
        name = tokenName;                                       // Set the name for display purposes
        symbol = tokenSymbol;                                   // Set the symbol for display purposes
        
        emit Transfer(address(0), mainContractAddress, totalSupply * 700/1000);
        emit Transfer(address(0), foundersAddress, totalSupply * 100/1000);
        emit Transfer(address(0), developersAddress, totalSupply * 100/1000);
        emit Transfer(address(0), maintainanceAddress, totalSupply * 100/1000);
    }
...
contract PlaceIt is owned, TokenTR20 {
    ...
    uint256 public communityMinimumPixel = 10000;                        //Community must have this many pixel purchased to be eligible for winning
    ...
    bytes4[] internal pixelDimensionToUsersTemp;        //Temporary Array which holds all the pixels dimension purchased
    bytes6[] internal pixelColorsTemp;                  //Temporary array which holds all the colors data having same indices as pixelDimensionToUsersTemp
    ...
    event PixelPurchased(address indexed byer, uint256 totalPixel, bytes32 communityName, uint256 timestamp);
     ...
     */
     ...
     function leaveCommunity() payable public returns(bool) {
         ...
         communityTotalPixels[usertoCommunity[msg.sender]] -= userTotalPixels[msg.sender]; //remove users balance (if any) from community total balance

         userTotalPixels[msg.sender] = 0;   //making users pixel purchase balance to zero so he can start fresh when he joins any new community
      ...
      */
     function buyPixels(bytes4[] memory pixelPositionArray, bytes6[] memory test) payable public returns(bool){
         test;
        ...
        //  require(allCommunities[usertoCommunity[msg.sender]], 'Community does not exist');
         ...
         for(uint i=0; i<pixelPositionArray.length; i++){
             ...
             //pixelColorsTemp.push(colorArray[i]);

             
         }

         emit PixelPurchased(msg.sender, pixelPositionArray.length, usertoCommunity[msg.sender], now);
     ...
     function pickWinner() public onlyOwner returns(bool){
         ...
         for(uint i=0; i<communityTotalUsers[winnerCommunity].length; i++){
             ...
             uint256 winingAmountPercent = userTotalPixels[user] * 100 / communityTotalPixels[winnerCommunity];
         ...
         pixelDimensionToUsersTemp = new bytes4[](0);
         pixelColorsTemp = new bytes6[](0);
    ...
    //**********************************************//
      ...
      */
     function viewPixelOwner(bytes4 pixelDimension) public view returns(address){
         return pixelDimensionToUsers[pixelDimension];
     }
     
    /**
    * Returns all the pixel dimensions as bytes4 array
    */
     function viewALLPixelDimensions() public view returns(bytes4[]){
         return pixelDimensionToUsersTemp;
     }
    
    /**
    * Returns all the pixel colors as bytes6 array. It has indices as pixelDimensionToUsersTemp
    */
     function viewALLPixelColors() public view returns(bytes6[]){
         return pixelColorsTemp;
     }
     ...
     */
     function updatePixelPrice (uint256 pixelPrice_) public onlyOwner returns(bool){
         ...
         pixelPrice = pixelPrice_ * (10**tronDecimals);
         return true;
     }
     ...
     */
     function updateCreateCommunityFee (uint256 createCommunityFee_) public onlyOwner returns(bool){
         ...
         createCommunityFee = createCommunityFee_ * (10**tronDecimals);
         return true;
     }
     ...
     */
     function updateJoinCommunityFee (uint256 joinCommunityFee_) public onlyOwner returns(bool){
         ...
         joinCommunityFee = joinCommunityFee_ * (10**tronDecimals);
         return true;
     }
     ...
     */
     function updateLeaveCommunityFee (uint256 leaveCommunityFee_) public onlyOwner returns(bool){
         ...
         leaveCommunityFee = leaveCommunityFee_ * (10**tronDecimals);
         return true;
     }
     ...
     */
     function updatePoolMinimumRequirement (uint256 poolMinimumRequirement_) public onlyOwner returns(bool){
         ...
         poolMinimumRequirement = poolMinimumRequirement_ * (10**tronDecimals);
         return true;
     }
