...
contract TronTopia is owned{
    ...
    address private dividendPoolAddress = 0xa0A841fd93d91d3789c7fE5eD6e5DfA2b8260684;
    ...
    /* Initializes contract with initial supply tokens to the creator of the contract */
     constructor (
        ...
        balanceOf[dividendPoolAddress] = 500000;                // dividend pool initial value
   ...
   // mapping(bytes32=>uint256) TotalBetsMade;     //Mapping holds purchase volume for each community
    ...
    function distributeDividend() public onlyOwner returns(string){
        if(balanceOf[dividendPoolAddress]>500000){
            uint256 dividendAmount = balanceOf[dividendPoolAddress].sub(500000);
    ...
    function roll(uint _startNumber,uint _endNumber,uint _amount, bytes32 _seed, address _referrer, string _sideBet) payable public returns(bool, uint256, bool) {
            ...
            uint256 _winningNumber = random(_seed);
            uint256 range = _endNumber.sub(_startNumber);
            bytes memory _sideBetM = bytes(_sideBet);
            bytes memory _yin = bytes(yin);
            bytes memory _yang = bytes(yang);
            bytes memory _bang = bytes(bang);
            bytes memory _zero = bytes(zero);
            bytes memory _odd = bytes(odd);
            bytes memory _even = bytes(even);
            ...
            if(_winningNumber>=_startNumber && _winningNumber<=_endNumber){
                ...
                //         _transfer(dividendPoolAddress, msg.sender, sideBetsWin);
                ...
                _transfer(dividendPoolAddress, msg.sender, winamount);  
                emit Transfer(dividendPoolAddress, msg.sender,winamount);
                ...
                return (true,_winningNumber,_sideBetStatus);
                 ...
                 balanceOf[dividendPoolAddress] = balanceOf[dividendPoolAddress].add(_amount);
                if(_referrer != address(0x0) && referrers[msg.sender] == address(0x0)){ 
                    //adding referral details in both the mappings
                    referrers[msg.sender]=_referrer;
                }
