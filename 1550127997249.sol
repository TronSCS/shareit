...
contract TronTopia is owned{
   ...
   // mapping(bytes32=>uint256) TotalBetsMade;     //Mapping holds purchase volume for each community
    ...
    function distributeDividend() public onlyOwner returns(string){
        if(communityPoolVolume>500000){
            uint256 dividendAmount = communityPoolVolume.sub(500000);
            uint256 dividend = dividendAmount.div(totalFrozenTopia);
    ...
    function setfrozenTopia(uint256 _value) public view returns(string){
