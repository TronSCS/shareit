...
contract TronTopia is owned{
   ...
   // mapping(bytes32=>uint256) TotalBetsMade;     //Mapping holds purchase volume for each community
    ...
    function userAvailabletoWithdrawTRX() public view returns(uint256){
        ...
        //calculating leaderboardDividendAvailable
        leaderboardDividendAvailable = leaderboardDividendPaid[msg.sender];

        return mainDividendAvailable + leaderboardDividendAvailable;

    }

    function userAvailabletoWithdrawTOPIA() public view returns(uint256){
    ...
    function roll(uint _startNumber,uint _endNumber,uint _amount, bytes32 _seed, address _referrer, string _sideBet, uint256 _sideBetvalue) payable public returns(bool, uint256, bool) {
            ...
            if(_amount>=1000){
                ...
                uint256 stage = mintTracker[mainContractAddress].div(100000000);
                stage = stage.mul(20);
            ...
            if(_winningNumber>=_startNumber && _winningNumber<=_endNumber){
                ...
                if(keccak256(_sideBetM) == keccak256(_yin)){
                    ...
                    if(firstNumber>lastNumber){
                        ...
                        emit SideBetWins(msg.sender, _winningNumber, _sideBetvalue,  sideBetsWin, _sideBet, true, now);
