...
contract TopiaToken is owned {
    ...
    /** 
        ...
        */
    function mintToken(address _user, uint256 _tronAmount)  public {

        //checking if the caller is whitelisted game contract
        require(whitelistMinter[msg.sender], 'Unauthorised caller');
    ...
    function updateMinter(address _newMinter, bool _status) public onlyOwner returns(string){
        
        require(_newMinter != address(0), 'Invalid minter');

        whitelistMinter[_newMinter] = _status;

        return "Minter updated";
    }
...
event dividendPaid(uint256 currentTRXBalance, uint256 totalDividendPaid);
    ...
    function availableMainDividends() public view returns(uint256){
        ...
        if(confirmedDividendForFreeze > 0){
            ...
            return (confirmedDividendForFreeze * sharePercentage / 100 / tronDecimals).sub(mainDividendPaid[msg.sender]);
        }
        else{
            return 0;
        }
