...
contract ClashRoyaleBalance {
    ...
    function coinsOf(address _addr) public view returns(
        ...
        uint hourlyProfit;
        uint time;
        uint[TYPES_WARRIORS] memory warriors = clashRoyale.warriorsOf(_addr);

        for (uint i = 0; i < TYPES_WARRIORS; i++) {
            hourlyProfit += warriors[i] * profit[i];
        }

        (treasury, spare, time) = clashRoyale.players(_addr);
