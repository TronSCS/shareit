...
contract TRONtopia_ultimate_dice is owned{
    ...
    */
    function addMultiplier (uint256[] memory data) public onlyOwner returns(string memory){
    ...
    function roll(uint _startNumber,uint _endNumber,uint _amount, bytes32 _seed, address _referrer, string memory _sideBet, uint256 _sideBetvalue) payable public returns(bool, uint256, bool, uint256) {
    ...
    function manualWithdrawTokens(uint256 tokenAmount) public view onlyOwner{
    ...
    function updateTopiaTokenContractAddress(address _newAddress) public onlyOwner returns(string memory){
