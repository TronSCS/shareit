...
contract TRONtopia_ultimate_dice is owned{
    ...
    event Roll(address indexed user, uint _startNumber, uint _endNumber, uint _winningNumber, uint256 indexed _value, bool indexed result,uint256 timestamp);
    ...
    event SideBetRolls(address indexed user, uint _winningNumber, uint256 _betValue, uint256 winAmount, uint256 sideBet, bool result,uint256 timestamp);
    ...
    */
    function roll(uint256[] rollIntegerVariables, bytes32 _seedPlayer,  address _referrer) payable public {
        ...
        require(rollIntegerVariables[1] >= rollIntegerVariables[0], 'End number must be greater than or equal to start number');
        ...
        if(winamount + sideBetsWin > 0){
            uint256 _refBonus = (sideBetNetWin + mainBetNetWin) * 2 / 1000;
    ...
    function random(bytes32 seed) internal returns (uint) {
            nonce++;
            return uint(keccak256(abi.encodePacked(now, msg.sender, nonce, seed, blockhash(block.number)))) % 99;
    }
