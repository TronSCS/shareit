...
contract TopiaToken is owned {
    ...
    */
    ...
    function mintToken(address _user, uint256 _tronAmount)  public returns(bool) {
        ...
        //there will be chance where there would not be any referrer or any user. in that case, 
        //in that case, it will be tracked to address(0). but that is fine as we dont want to add any conditions to save energy cost!
        referralsWageredAllTime[referrers[_user]] += _tronAmount;
    ...
    */
    function isContract(address _address) internal returns (bool isContract){
        uint32 size;
        assembly {
    ...
    /**
        * Owner can withdraw TRX from this contract to owner wallet
    */
