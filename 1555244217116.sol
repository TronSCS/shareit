...
contract DankPotROI{
    ...
    uint256 public interest = 3;  
    uint256 public minutesElapse = 60*24; // production 60 * 24 
    uint256 public minimum = 1000000; // minimum deposit = 1 trx
   
    uint256 private referralDivRate = 20; // 2% Referral  
    uint256 private devDivRate = 70; // 5% to Development and 2% to Marketing
    ...
    function deposit(address _referredBy) public payable {
       ...
       // add new despoit to current deposit, and update joined timer
    ...
    */
    function getBalance(address _address) view public returns (uint256) {
        if(joined[_address]>0)
        {
            ...
            uint256 percent = investments[_address].mul(interest).div(100); // how much to return, step = 3 is 3% return
            ...
            uint256 balance = difference.sub(withdrawals[_address]); // calculate how much can withdraw now
    ...
    * @param _investor The address to query the balance of.
    * @return An uint256 representing the amount owned by the passed address.
    */
    function getAllData(address _investor) 
    public view returns (uint256 withdrawns, uint256 withdrawable, uint256 investment, uint256 referral) {
        return (withdrawn[_investor],getBalance(_investor),investments[_investor],referrer[_investor]);
    }
