...
contract TopiaToken is owned {
    ...
    */
    ...
    /**
        ...
        Owner only can only withdraw less than refPool amount, which is leftover from the referral withdrawals
    */
    function claimOwnerRefs(uint256 amount) public onlyOwner returns (string){
        require(amount <= refPool, 'Owner can not withdraw more than refPool value');
...
46903153
    ...
    function distributeMainDividend() public  returns(uint256){
        ...
        for(uint i=0; i < totalGameContracts; i++){
            ...
            if(status){
                ...
                //we deduct 1% of finalAmount from itself for Leaderboard distribution
                uint256 leaderBoardShare = amount / 100;
                //we deduct RefPool share as well
                uint256 refPoolShare = amount * refPoolPercentSUN / 100000000;  //refPoolPercentSUN is in SUN
                uint256 refAndLeadeardboard = leaderBoardShare + refPoolShare;
                uint256 finalAmount = amount - refAndLeadeardboard;
                ...
                require(InterfaceGAMES(whitelistCallerArray[i]).requestDividendPayment(bronzrTierAmount+silverTierAmount+goldTierAmount+refAndLeadeardboard), 'could not transfer trx');
        ...
        if(totalDividend > totalReductions){
            ...
            mainDiviendPaidAllTime += finalDividendAmount * 98 / 100;
    ...
    */
    function availableMainDividends() public view returns(uint256){
        ...
        if(mainDiviendPaidAllTime > previousDivAmountWithdrawal[msg.sender]){
            ...
            uint256 totalFrozenTopia = topiaFrozenAtDivDistribution - totalFrozenTopiaTracker[msg.sender];
            uint256 totalNoOfDivPaid = totalDividendsPaidNumber - noOfDivPaidAfterFreeze[msg.sender];
            ...
            if(bronzeTopiaUser[msg.sender] > 0){
                ...
                sharePercentage = bronzeTopiaUser[msg.sender] * 100 * tronDecimals / (totalFrozenTopia / totalNoOfDivPaid) ;
            }
            //silver tier
                ...
                //so tronDecimals is multiplied in sharePercentage,which was then divided in total amount in below equasion.
                //(totalFrozenTopia / totalNoOfDivPaid) at the end is avearage of all topia fronzen at time of div distribution
                sharePercentage = silverTopiaUser[msg.sender] * 100 * tronDecimals / (totalFrozenTopia / totalNoOfDivPaid);
            }
            //gold tier
                ...
                //so tronDecimals is multiplied in sharePercentage,which was then divided in total amount in below equasion.
                //(totalFrozenTopia / totalNoOfDivPaid) at the end is avearage of all topia fronzen at time of div distribution
                sharePercentage = goldTopiaUser[msg.sender] * 100 * tronDecimals / (totalFrozenTopia / totalNoOfDivPaid);
            }
    ...
    */
    function withdrawDividendTRXandTopia() public {
        ...
        if(availableMainDividend + availableLeaderboardDividend > 0){
            ...
            totalFrozenTopiaTracker[msg.sender] = topiaFrozenAtDivDistribution;

            //this will track all the dividend distribution attempts.
            noOfDivPaidAfterFreeze[msg.sender] = totalDividendsPaidNumber;
    ...
    */
    function unfreezeTopia() public returns(bool){
        ...
        if(outstandingDiv > 0){
            ...
            //no need to update those freeze tier trackers, as it will be updated when user will freeze again..
    ...
    */
    ...
    /**
        Function returns all topia frozen in bronze, silver and gold tiers ENTIRELY
    */
