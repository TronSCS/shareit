...
contract TRONtopia_ultimate_dice is owned{
    ...
    uint256 private constant minimumTokenRequirement = 10000000000;      //User must have at least 100 Tokens to be eligible to receive winnings
    uint256 private constant poolMinimumRequirement = 1000000000000;     //1 Million
               
    uint256 public maxBetAmount = 500000;
    uint256 public minimumSideBetAmount = 25;
    uint256 private maxWinDivisibleAmount = 50;

    //side bet jackpot variables
    uint256 private sideBetJackpotAmountMultiplier = 1000;   // 1000 is multiplier to calculate trx for sidebet jackpot
    ...
    uint256 private maxSidebetJackpotAmount = 250000000000;   // 250K trx is the maximum sidebet jackpot amount
    ...
    */
    function roll(uint256[] rollIntegerVariables, bytes32 _seedPlayer,  address _referrer) payable public {
        ...
        //checking conditions
        require(msg.value == (rollIntegerVariables[2].add(rollIntegerVariables[3])) * 1e6 && rollIntegerVariables[2] >= 10, 'Invalid _amount');
        require((rollIntegerVariables[2] + rollIntegerVariables[3]) <= maxBetAmount, 'Bet amount too large');
        ...
        require(_referrer != msg.sender, 'User can not refer himself');
        if(rollIntegerVariables[3] > 0) { require(rollIntegerVariables[3] >= minimumSideBetAmount, 'If sidebet is placed, then it can not be les than 25 trx'); }
        ...
        if(_winningNumber>=rollIntegerVariables[0] && _winningNumber<=rollIntegerVariables[1]){
            ...
            //range = rollIntegerVariables[1] - rollIntegerVariables[0], which need to be looked up in multipliersData
            winamount = rollIntegerVariables[2] * multipliersData[rollIntegerVariables[1] - rollIntegerVariables[0]] * 100;  
        ...
        if(_winningNumber>=rollIntegerVariables[0] && _winningNumber<=rollIntegerVariables[1]){
            ...
            if((rollIntegerVariables[1] - rollIntegerVariables[0]) < 5){
    ...
    */
    function random(bytes32 seed) internal returns (uint) {
            nonce++;
            return uint(keccak256(abi.encodePacked(now, msg.sender.balance, address(this).balance, nonce, block.coinbase,  gasleft(), blockhash(block.number - 1), seed))) % 100;
    }
    ...
    */
    function randomForSidebetJackpot(uint256 sideBetAmount) internal {
        ...
        //this function does not have more parameters as like in above random function (to save gas), as this is not as strict as that one
        uint randomNumber = uint(keccak256(abi.encodePacked(now, msg.sender.balance, nonce, block.coinbase,  gasleft(), blockhash(block.number - 1)))) % sideBetJackpotMaxOdd;
        ...
        if(randomNumber == sideBetJackpotFixNumber){
            ...
            emit SideBetJackpot(msg.sender, randomNumber, sideBetJackpotAmount );
        }
    }
    ...
    */
    ...
    /**
        Function allows owner to update the DIVIDEND contract address
    */
    function updateDividendContractAddress(address _newAddress) public onlyOwner returns(string){
    ...
    */
    ...
    /**
        Function to change max bet amount and max bet divisible amount.
    */
    function updateMinimumSideBetAmount(uint256 minimumSideBetAmount_) public onlyOwner returns(string){
        
        minimumSideBetAmount = minimumSideBetAmount_;
    ...
    */
    function displayAvailableDividend() public view returns (bool, uint256){
        ...
        if(address(this).balance >= dividendThreshold ){
            return (true, address(this).balance - dividendThreshold );
        }
        else{
            return (false, dividendThreshold - address(this).balance );
        }
    }
